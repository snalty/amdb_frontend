...
<template>
      ...
      <div class="flex h-16 items-center justify-between">
        ...
        <div class="hidden md:block">
          <div class="ml-10 flex items-baseline space-x-4">
            <div class="bg-white border-gray-200 shadow-sm flex border-1 rounded-full">
              <div id="search">
                <select v-model="genome" class="bg-inherit block w-full rounded-md border-0 p-1.5 m-1 w-min text-gray-900">
                  <option selected>hg19</option>
                  <option>hg38</option>
                </select>
              </div>
              <input v-model="search" class="w-96" placeholder="Search for position e.g 1-55523855-G-A" @keyup.enter="validateSearch(genome, search)" />
  ...
  <footer class="fixed bottom-0 w-full flex justify-center">
        <div class="bg-gray-200 rounded-lg p-2 px-16 m-3 ">
